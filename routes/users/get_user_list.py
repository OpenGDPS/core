...
async def get_friends():
    ...
    if type == "0":
        ...
        for result in fetched_requests:
            ...
            color_2 = account[26]
            isNew = result[2]
            str = str + f"1:{username}:2:{userID}:9:{icon_cube}:10:{color_1}:11:{color_2}:14:0:15:0:16:{userID}:18:0:41:{isNew}|"
        cursor.execute(f'UPDATE friends SET isNew = 0 WHERE user1 = {accId}')
        conn.commit()
        if str == "": return "-2"
        else: return f"{str}"
